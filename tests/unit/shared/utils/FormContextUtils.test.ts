...
describe("LibraryUtils field manipulation", () => {
  it("setFieldsVisible toggles only the named attributes", () => {
    ...
    FormContextUtils.setFieldsVisible(formContext, ["name", "revenue"], false);
  ...
  it("setFieldsDisabled tolerates attributes missing from the form", () => {
    ...
    expect(() =>
      FormContextUtils.setFieldsDisabled(formContext, ["name", "not_on_form"], true)
    ).not.toThrow();
  ...
  it("setFieldsRequired sets the requirement level", () => {
    ...
    FormContextUtils.setFieldsRequired(formContext, ["telephone1"], "required");
  ...
  it("lockAllFields disables everything except the allow-list", () => {
    ...
    FormContextUtils.lockAllFields(formContext, { except: ["ownerid"] });
    expect(controls.get("name")!.setDisabled).toHaveBeenCalledWith(true);
  ...
  it("unlockAllFields re-enables fields", () => {
    const { formContext, controls } = makeFakeForm(["name"]);
    FormContextUtils.unlockAllFields(formContext);
  ...
  it("set/clearFieldNotification target the attribute's controls", () => {
    ...
    FormContextUtils.clearFieldNotification(formContext, "telephone1", "phone-required");
  ...
  it("addFieldNotification passes rich options to the attribute's controls", () => {
    ...
    FormContextUtils.addFieldNotification(formContext, "websiteurl", options);
  ...
  it("addFieldNotification is cleared by clearFieldNotification, no separate remover", () => {
    ...
    FormContextUtils.clearFieldNotification(formContext, "websiteurl", "site-rec");
  ...
  it("set/clearFormNotification delegate to the form ui", () => {
    ...
    expect(FormContextUtils.clearFormNotification(formContext, "warn-1")).toBe(true);
    expect(clearFormNotification).toHaveBeenCalledWith("warn-1");
  });

  it("getFormType maps the raw integers", () => {
    expect(FormContextUtils.getFormType(makeFakeForm([], 1).formContext)).toBe("create");
    expect(FormContextUtils.getFormType(makeFakeForm([], 2).formContext)).toBe("update");
    expect(FormContextUtils.getFormType(makeFakeForm([], 3).formContext)).toBe("readonly");
    expect(FormContextUtils.getFormType(makeFakeForm([], 6).formContext)).toBe("bulkedit");
  });
});
