...
| A call rejects "not supported on the CRM 8.x host" | The V8 adapter maps the 8.x subset and rejects the rest loudly, never silently no-ops | Treat as a host capability ceiling, not a bug. The whole V8 path is best-effort: mock-tested and contract-verified at the v8.2 API line, still untested against a live 8.x server (README) |
