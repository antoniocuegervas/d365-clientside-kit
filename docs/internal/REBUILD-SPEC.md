...
6. Look up app in registry; render app inside `ViewModelContextProvider` (a
   full-page launch, marked `fullPage`, gets the shell's Back bar above a
   bounded scrolling app region on the web client)
7. Full-viewport sizing is CSS-driven (html/body/#container at 100%), no JS
   resize handling
8. Unmount on `pagehide` (browsers deprioritize `beforeunload` and it blocks
